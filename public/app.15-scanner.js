    ...
    <div class="scan-body">
      ...
      <div class="scan-controls">
        ${supported
          ...
          : `<span class="scan-note">Live camera scan isn’t supported on this browser — type a code or use a scanner gun.</span>`}
...
// data-act: begin live camera scanning.
async function scanStartCamera() {
  ...
  try {
    if (BarcodeDetector.getSupportedFormats) {
      ...
      const f = fmts.filter(x => sup.includes(x));
      fmts = f.length ? f : sup;
    }
  ...
  catch (e) { showToast('Scanner unavailable — use the box below', 'error'); return; }

  let stream;
  try { stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: 'environment' } } }); }
  catch (e) {
    showToast('Camera blocked — allow access or use a scanner gun', 'error');
    if (status) status.textContent = 'Camera unavailable. Type a code or use a scanner gun.';
    return;
  }
  ...
  const state = { running: true, raf: 0, lastCode: '', lastAt: 0 };
  const loop = async () => {
    ...
    try {
      ...
      if (codes && codes.length) {
        ...
        if (raw && (raw !== state.lastCode || now - state.lastAt > 900)) {
          state.lastCode = raw; state.lastAt = now;
          handleScanCode(raw);
        }
      }
